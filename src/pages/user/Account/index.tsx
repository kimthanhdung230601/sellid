  ...
  const {
    ...
    onSuccess: (fetchedData) => {
      ...
      if (money) {
        ...
        const result =
          parseInt(fetchedData?.data[0].money, 10) -
          parseInt(decryptedMoney, 10);
        if (result > 0) {
          setIsModalOpen(true);
          setMoney(result);
    ...
    <div className={styles.wrapper}>
          ...
          <Col span={10} xxl={10} xl={10} lg={24} md={24} sm={24} xs={24}>
            <div className={styles.accountAbout}>
              ...
              {isFirstFetchFetching ? (
                <div className={styles.aboutName}>loading...</div>
              ) : (
                <div className={styles.aboutName}>
                  {userInfor?.data[0].username}
                </div>
              )}
              {isFirstFetchFetching ? (
                <div className={styles.aboutEmail}>loading...</div>
              ) : (
              ...
              <div className={styles.aboutSpending}>
                <div className={styles.spending}>
                  ...
                  {isFirstFetchFetching ? (
                    <div className={styles.spendingNumber}>loading...</div>
                  ) : (
                    <div className={styles.spendingNumber}>
                      {formatCurrency(parseInt(userInfor?.data[0].money, 10))}{" "}
                      VNĐ
                    </div>
                  )}
                </div>
          ...
          <Col span={14} xxl={14} xl={14} lg={24} md={24} sm={24} xs={24}>
            <div className={styles.inforWrap}>
              ...
              <div className={styles.infor}>
                <Image
                  src={`https://api.vietqr.io/Mbbank/107069099999/0/naptien%20${userInfor?.data[0].username}/vietqr_net_2.jpg?accountName=LE%20THI%20THU%20HA&fbclid=IwAR32mVZcLPmA_8TLyVwonR7-ZgX5mXD2MooKJoNCrQNhiUd5kSBZwoyATak`}
                ...
                <div className={styles.inforBank}>
                  ...
                  <div className={styles.inforItem}>
                    ...
                    <div className={styles.value}>
                      naptien {userInfor?.data[0].username}
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles.notiWrap}>
                <li className={styles.noti}>
                  Quý khách vui lòng nhập đúng nội dung chuyển khoản để nạp
                  tiền, nội dung chuyển khoản có cú pháp{" "}
                  <b style={{ color: "#c4212a" }}>
                    {"[naptien <tên username>]"} ví dụ "naptien{" "}
                    {userInfor?.data[0].username}"
                  </b>{" "}
                  . Nếu nhập sai nội dung chuyển khoản, tài khoản sẽ{" "}
                  <b style={{ color: "#c4212a" }}>KHÔNG</b> nạp được tiền.
                </li>
                <li className={styles.noti}>
                  Quý khách cũng có thể quét mã QR để chuyển khoản với nội dung
                  đã được nhập sẵn.
                </li>
                <li className={styles.noti}>
                  Sau khi chuyển khoản thành công, tài khoản sẽ được tự động cập
                  nhật trong khoảng 1 phút{" "}
                </li>
              </div>
            </div>
          </Col>
      ...
      >
        ...
        <div style={{ fontSize: "16px", marginTop: "20px" }}>
          Chúc mừng bạn đã nạp thành công số tiền{" "}
          <span style={{ color: "#69AD3A", fontWeight: "600" }}>
            {" "}
            {formatCurrency(money)} VNĐ
          </span>
        </div>
      </Modal>
    </div>
