...
export default function Home() {
  ...
  const { data: product, isFetching } = useQuery(
    ["product", idCategory, currentPage],
    () => getAllProduct(currentPage, idCategory)
  );
        ...
        <>
          <div className={style.category}>
            <span
              ...
              onClick={() => handleSelected("all", "all")}
            >
              Tất cả
            </span>
          </div>
            ...
            {category?.data.map((item: any, index: number) => {
              return (
                <div className={style.category} key={item.id}>
                  <span
                    className={
                      param.id === item.id && JSON.stringify(param) !== "{}"
                        ? ` ${style.categoryActive}`
                        : `${style.categoryName}`
                    }
            ...
            <div className={style.rowWrap}>
              ...
              >
                {isFetching ? (
                  ...
                  <div className={style.noti}>
                    <WarningFilled
                      style={{ fontSize: "30px", color: "#167fff" }}
                    />
                    <div className={style.notiText}>
                      Sản phẩm trong chuyên mục này đã bán hết
                    </div>
                  </div>
                )}
              </Row>
              {isFetching ? null : product?.status === "success" ? (
